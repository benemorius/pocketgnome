...
- (void)addRule: (Rule*)rule {
    if(rule != nil)
        ...
        log(LOG_ERROR, @"addRule: failed; rule is nil");
}
...
- (void)insertRule: (Rule*)rule atIndex: (unsigned)index {
    if(rule != nil && index >= 0 && index <= [_rules count])
        ...
        log(LOG_ERROR, @"insertRule:atIndex: failed; rule %@ index %d is out of bounds", rule, index);
}
...
- (void)replaceRuleAtIndex: (int)index withRule: (Rule*)rule {
    ...
    if((rule != nil) && (index >= 0) && (index < [self ruleCount])) {
        ...
        log(LOG_ERROR, @"replaceRule:atIndex: failed; either rule is nil or index is out of bounds");
}
